...
import {BrowserWindow, ipcRenderer} from "../../platform";
...
class WfcImpl {
    ...
    init(args = []) {
        ...
        //     protoProxy.setDBPath(process.cwd());
        // }
        // protoProxy.setAppName('mychat');
    ...
    connect(userId, token) {
        ...
        self.userId = userId;
        return protoProxy.invoke('connect', userId, token);
    ...
    getMessages(conversation, fromIndex, before = true, count = 20, withUser = '', contentTypes = []) {
        let protoMsgsStr = protoProxy.invoke('getMessages', JSON.stringify(conversation), contentTypes, fromIndex, before, count, withUser);
    ...
    searchMessage(conversation, keyword) {
        let result = protoProxy.invoke('searchMessage', JSON.stringify(conversation), keyword);
    ...
    searchMessageEx(conversation, keyword, desc, limit, offset) {
        let result = protoProxy.invoke('searchMessageEx', JSON.stringify(conversation), keyword, desc, limit, offset);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    searchMessageByTypes(conversation, keyword, contentTypes, desc, limit, offset) {
        ...
        let result = protoProxy.invoke('searchMessageByTypes', JSON.stringify(conversation), keyword, contentTypes, desc, limit, offset);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    searchMessageByTypesAndTimes(conversation, keyword, contentTypes, startTime, endTime, desc, limit, offset) {
        ...
        let result = protoProxy.invoke('searchMessageByTypesAndTimes', JSON.stringify(conversation), keyword, contentTypes, startTime.toString(), endTime.toString(), desc, limit, offset);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    searchMessageEx2(conversationTypes, lines, contentTypes, keyword, fromIndex, desc, count) {
        ...
        let result = protoProxy.invoke('searchMessageEx2', conversationTypes, lines, contentTypes, keyword, fromIndex, desc, count);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    async sendMessageEx(message, toUsers = [], preparedCB, progressCB, successCB, failCB) {
        ...
        if (message.messageContent instanceof MediaMessageContent && (message.messageContent.localPath || message.messageContent.file) && !message.messageContent.remotePath && this.isSupportBigFilesUpload()) {
            let insertedMsg = this.insertMessage(message.conversation, message.messageContent, MessageStatus.Sending, true, 0);
            ...
            if (message.messageContent.localPath && !message.messageContent.file) {
                let buffer = require('fs').readFileSync(message.messageContent.localPath)
                //let ab = buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength);
                message.messageContent.file = new File([buffer], message.messageContent.localPath);
            }
            // image/jpeg
            this._uploadFile(insertedMsg, message.messageContent.file, message.messageContent.mediaType, (uploaded, total) => {
    ...
    async _uploadFile(message, file, mediaType, progressCB, successCB, failCB) {
        ...
        if (message.conversation.type === ConversationType.SecretChat) {
        ...
        this.getUploadMediaUrl(fileName, mediaType, `application/octet-stream`, (uploadUrl, remoteUrl, backUploadUrl, serverType) => {
            let xhr;
            if (serverType === 0) {
                // 内置存储，不支持大文件上传
                ...
                let token = ss[1];
                let key = ss[2];
                xhr = this._uploadXMLHttpRequest(message.messageId, fileName, remoteUrl, progressCB, successCB, failCB);
    ...
    getAuthorizedMediaUrl(messaggeUid, mediaType, mediaPath, successCB, failCB) {
        protoProxy.invoke('getAuthorizedMediaUrl', messaggeUid, mediaType, mediaPath, successCB, failCB);
    }
    ...
    insertMessage(conversation, messageContent, status, notify = false, serverTime = 0) {
        let protoMessageContent = messageContent.encode();
        let mStr = protoProxy.invoke('insertMessage', JSON.stringify(conversation), self.userId, JSON.stringify(protoMessageContent), status, notify, serverTime);
        return Message.fromProtoMessage(JSON.parse(mStr));
    }
    ...
    async uploadMedia(fileName, data, mediaType, successCB, failCB, progressCB) {
        ...
        protoProxy.invokeAsync('uploadMedia', fileName, data, mediaType,
            (remoteUrl) => {
                if (successCB) {
                    successCB(remoteUrl);
                }
